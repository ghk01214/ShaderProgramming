...
{
	//(x, y, z, vx, vy, vz, emit_time, life_time, amp, period)
...
void Renderer::Lecture3_Particle()
{
	...
	glVertexAttribPointer(attribVelocity, 3, GL_FLOAT, GL_FALSE, sizeof(float) * 10, reinterpret_cast<GLvoid*>(sizeof(float) * 3));

	int attribEmitTime{ glGetAttribLocation(shader, "emit_time") };
	glEnableVertexAttribArray(attribEmitTime);
	glVertexAttribPointer(attribEmitTime, 1, GL_FLOAT, GL_FALSE, sizeof(float) * 10, reinterpret_cast<GLvoid*>(sizeof(float) * 6));

	int attribLifeTime{ glGetAttribLocation(shader, "life_time") };
	glEnableVertexAttribArray(attribLifeTime);
	glVertexAttribPointer(attribLifeTime, 1, GL_FLOAT, GL_FALSE, sizeof(float) * 10, reinterpret_cast<GLvoid*>(sizeof(float) * 7));

	int attribAmp{ glGetAttribLocation(shader, "amp") };
	glEnableVertexAttribArray(attribAmp);
	glVertexAttribPointer(attribAmp, 1, GL_FLOAT, GL_FALSE, sizeof(float) * 10, reinterpret_cast<GLvoid*>(sizeof(float) * 8));

	int attribPeriod{ glGetAttribLocation(shader, "period") };
	glEnableVertexAttribArray(attribPeriod);
	glVertexAttribPointer(attribPeriod, 1, GL_FLOAT, GL_FALSE, sizeof(float) * 10, reinterpret_cast<GLvoid*>(sizeof(float) * 9));
